...
class TestUserService(BaseTestCase):
    ...
    def test_single_user_no_id(self):
        """asegurese de que se arroge un  error si no se proporciona una identificacione"""
        with self.client:
    ...
    def test_single_user_incorrect_id(self):
        """asegurando de que se arroje un error si la identificacion no existe"""
        with self.client:
    ...
    def test_all_users(self):
        ...
        with self.client:
            ...
            self.assertIn('Brandux JUarez', data['data']['users'][0]['username'])
            self.assertIn(
                'branduxjuarez@upeu.edu.pe', data['data']['users'][0]['email'])
            self.assertIn('Didier', data['data']['users'][1]['username'])
            self.assertIn(
    ...
    def test_main_add_user(self):
        """Asegurando que un nuevo ususario pueda ser agregado a 
        la db mediante un request"""
        with self.client:
            response = self.client.post(
                '/',
                data=dict(username= 'brandux', email= 'branduxjuarez@upeu.edu.pe'),
